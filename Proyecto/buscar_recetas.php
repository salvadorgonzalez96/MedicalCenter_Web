...
<body>
    ...
    <div class="container">
        ...
        <table id="data-table" class="table table-condensed table-striped">
            <thead>
            <tr>
                <th width="7%">No. Receta</th>
                <th width="10%">Fecha Realizada</th>     
                <th width="25%">Nombre Paciente</th>
                <th width="6%">Edad</th>
        ...
        <?php
                $sql="select *
                        FROM tbl_receta;";//SQL que van a modificar
                $result=mysqli_query($conexion,$sql);
                while($row=mysqli_fetch_assoc($result)){
                    $invoiceDate = date("d/M/Y", strtotime($row["receta_fecha"]));
                    echo '
                    <tr>
                        <td>'.$row["receta_codigo"].'</td>
                        <td>'.$invoiceDate.'</td>
                        <td>'.$row["receta_cliente"].'</td>
                        <td>'.$row["receta_edad"].'</td>
                        <td>'.$row["doctor_nombre"].'</td>

                        <td><a href="imprimir_receta.php?update_id='.$row["receta_codigo"].'"  TARGET="_self" title="Imprimir Receta"><div class="btn btn-primary"><span><i><img src="img/print.png"/></i></span></div></a></td>
                    </tr>';
                }
