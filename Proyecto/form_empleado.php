...
<body>
	...
	<div id="all"align='center'><h2>Formulario para Mantenimiento de Empleado</h2>
...
**************************************************************************************************************-->
	...
	<div align="center"><h1><b>Listado de Empleados</b></h1>
		<table name='tabla' id="tabla" align="center" border="1" width="80%">
			...
			<tbody>
				<?php 
					$sql="select 
					empleado_cedula as cedula,
					empleado_nombre as nombre,
					empleado_apellido1 as ape1,
					empleado_apellido2 as ape2,
					empleado_estado as estado
 					from tbl_empleado";
					$result=mysqli_query($conexion,$sql);
					$x=1;
					while($row=mysqli_fetch_assoc($result))
					{
						...
						$color=($row["estado"]=="activo"?"#F3E2A9":"#D0F5A9");
						...
						<tr bgcolor='".$color."'>
							...
							<td>".$row["ape1"]."</td>
							<td>".$row["ape2"]."</td>
							<td align='center'><img src='img/configurar.png' onClick='return llenar(\"".$row["cedula"]."\")'></td>
						</tr>";
						$x++;
